...
Page({
  ...
  loadFromApi () {
    request(api.healers, { auth: false }).then((list) => {
      if (list && list.length) {
        this.setData({
          healers: list.map(h => ({
            id: h.id,
            name: h.name,
            title: h.title || '',
            quote: h.intro ? '"' + h.intro + '"' : '',
            tags: h.tags || [],
            bg: 'linear-gradient(135deg,#C9B59A,#8a7558)'
          }))
        })
        this.applyFilter()
      }
  ...
  // 按 catIdx 过滤（catIdx=0 全部；其余按 tag 匹配，去空格后包含即命中）
  applyFilter () {
    const { catIdx, cats, healers } = this.data
    const cat = cats[catIdx].replace(/ /g, '')
    const filtered = catIdx === 0 ? healers : healers.filter(h => (h.tags || []).some(t => t.replace(/ /g, '').indexOf(cat) > -1))
